...
12. **Opt-in debug logging for auth troubleshooting**: two Debug-level, off-by-default logging hooks exist for tracing exactly what a token decoded to and what each plugin saw — both are dev / incident-response tooling, **never enable on a shared or production cluster** (they log PII: email, name, preferred_username, full header sets).
    - `debug: {log_plugin_input: true}` at the config's top level (`config.DebugConfig`, restart-only — same reload rule as `listener.*`) logs one `pipeline: plugin input` line per plugin per request: direction/method/path/host, redacted headers (`redact.Headers`), and — once an auth plugin has run — `pctx.Identity`'s subject/client-ID/scopes plus any `contracts.ClaimsCarrier` claims. Threaded into `pipeline.WithDebugPluginInput` from both `cmd/authbridge-proxy` and `cmd/authbridge-envoy`'s `buildPipelines`.
    - `debug_log_claims: true` on jwt-validation's own plugin config (hot-reloadable — it's an ordinary plugin config field) logs one `jwt-validation: decoded token claims` line per `Verify` call with the **full** decoded claim set (registered + `Extra`, redacted via `redact.JSON`) — on both the success path and the audience-mismatch failure path, since that failure is exactly when you need to see what the token actually contained. This is the tool that would have shown a missing `sub` claim or an unexpected `azp` immediately, instead of needing to decode the JWT by hand outside the cluster.
