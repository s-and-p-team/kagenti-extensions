#!/usr/bin/env bash
# uc1-integration-enable.sh — turn on the infra #646 needs that isn't wired by default:
...
#
# All three ship as real, merged code (PRs #752/#754) but — per #754's own description —
# "both are opt-in deployments, so with the default manifests no events are produced or
# consumed." This script is what opts them in, on a Kind cluster, for the uc1-integration
# runbook (see uc1-integration-runbook.md).
#
# Mirrors aiac/k8s/opa-kind-enable.sh's shape: idempotent, env-var configured, never edits a
# checked-in chart file — the Keycloak StatefulSet image swap is a live `kubectl set image`
# patch, not a chart edit (see Step 3 below; reverted by uc1-integration-restore.sh).
#
# Usage:
#   ./uc1-integration-enable.sh                # do all three steps
#   ./uc1-integration-enable.sh --stack-only   # just the AIAC stack
#   ./uc1-integration-enable.sh --broker-only  # just the NATS broker
#   ./uc1-integration-enable.sh --spi-only     # just the Keycloak SPI listener
#
# Env vars:
...
load_image_to_kind() {
  ...
  if [ "$CONTAINER_RUNTIME" = "podman" ]; then
    local tar_file
    tar_file="$(mktemp "${TMPDIR:-/tmp}/uc1-integration-image.XXXXXX")"
    TMPFILES+=("$tar_file")
...
step_broker() {
  ...
  echo "==> [broker] aiac-event-broker up. step_spi sets NATS_URL on the Keycloak StatefulSet to the namespace-qualified FQDN — see below."
}
...
step_spi() {
  ...
  local build_dir
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/uc1-integration-kc-image.XXXXXX")"
  TMPFILES+=("$build_dir")
...
Next: aiac/demo/use-cases/uc1-onboarding's "make init" (make keycloak && make prereqs && make
clear && make setup), then uc1-integration-driver.sh — see uc1-integration-runbook.md.
